...
{
	var mapper = $('mapper');
	var url = 'ajax.php'+element.value;
	if (!my_AJAX.isCached(url))
		outLoadingText('mapperarea');
	my_AJAX.GETupload(url, function (text) {mapper.innerHTML = text;});
}
...
function outLoadingText(div_id)
{
	...
	if (txt)
	{
	    var div = insertElement(txt, 'DIV', 'loading');
        var s = div.style;
		s.position='absolute';
        s.left= (txt.offsetWidth/2 - 30) + 'px';
        s.top = '10px';
		div.innerHTML = outText('Loading...');
	}
}
...
function renderInstance(elementid, selected)
{
	...
	var mapper = $(elementid);
	if (mapper == 0)
		return;
	var text;
	var map = eval(mapData);

	map.width  = map.imageX*scale;
	var rect = getBounds(mapper.offsetParent);
	map.height = map.imageY*scale;
	var rect = getBounds(mapper.offsetParent);
	text =
	'<table class="map" border=1 width=' + (map.width + 10) + 'px>' +
	'<tbody><tr><td class=mapname id=mappername>' + map.header +'</td></tr>' +
	'<tr><td width=' + map.width + 'px height=' + (map.height + 4) + 'px align=left valign=top>' +
	'<div id=mapperarea style="font-size: 10px; position: relative; left: 0px; top: 0px;">' +
	'<img src=' + map.image + ' width=' + map.width + 'px height=' + map.height + 'px>\n';
	...
	for (var i = 0; i < count; i++)
	{
		...
		text +='<img src="' + img + '" style="position: absolute; border: 0px; left: ' + x.toFixed(0) + 'px; top: ' + y.toFixed(0) + 'px;" ' + tip + '>\n';
