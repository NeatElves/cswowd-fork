...
function tt_updatePosition()
{
    ...
    css.left = (x + tt_scrlX)+'px';
    css.top  = (y + tt_scrlY)+'px';
}
...
function tt_SetOpa(oElem, nOpacity)
{
	var p = tt_getOpaSettings();
	if (p=='filter')
		tt_SetOpa = new Function('oElem', 'nOpacity', 'if (nOpacity >= 100) {oElem.style.filter = ""; return;} var oAlpha = oElem.filters["DXImageTransform.Microsoft.alpha"] || oElem.filters.alpha; if (oAlpha) oAlpha.opacity = nOpacity;else {oElem.style.zoom = 1;oElem.style.filter = "progid:DXImageTransform.Microsoft.Alpha(opacity="+nOpacity+")";}');
	else if (p)
		tt_SetOpa = new Function('oElem', 'nOpacity', 'oElem.style.'+p+' = nOpacity/100;');
	else
		tt_SetOpa = new Function(oElem, nOpacity);
	return tt_SetOpa(oElem, nOpacity);
}
