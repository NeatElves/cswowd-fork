...
{
  global $cache_array, $dDB;
  // Получаем рефренс лут
  if (!isset($cache_array['rl'.$entry]))
      $cache_array['rl'.$entry] = $dDB->select('SELECT `entry` AS ARRAY_KEY, `ChanceOrQuestChance`, `groupid`, `mincountOrRef`, `maxcount`, `lootcondition`, `condition_value1`, `condition_value2` FROM `reference_loot_template` WHERE `item`=?d', $entry);
  return $cache_array['rl'.$entry];
}
function getFactionTemplates($entry)
{
  global $cache_array, $wDB;
  if (!isset($cache_array['ft'.$entry]))
      $cache_array['ft'.$entry] = $wDB->selectCol('SELECT `id` FROM `wowd_faction_template` WHERE `faction` = ?d', $entry);
  return $cache_array['ft'.$entry];
}
function getPlayerSpells($guid)
{
  global $cache_array, $cDB;
  if (!isset($cache_array['ps'.$guid]))
      $cache_array['ps'.$guid] = $cDB->select('SELECT `spell` AS ARRAY_KEY FROM `character_spell` WHERE `guid` = ?d AND `disabled` = 0', $guid);
  return $cache_array['ps'.$guid];
}
//==============================================================================
// Callback functions
//==============================================================================
...
class QuestReportGenerator extends ReportGenerator{
 ...
 {
  $this->doRequirest('(`ReqItemId1`= ?d OR `ReqItemId2`= ?d OR `ReqItemId3`= ?d OR `ReqItemId4`= ?d) AND `quest_template`.`entry` <> ?d', $entry, $entry, $entry, $entry, $giveQuest);
 }
