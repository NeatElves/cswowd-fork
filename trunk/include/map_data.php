...
// Производит поворот координат на угол
...
function defaultMapRenderCallback($data, $x, $y)
{
   ...
   if ($area_data = getArea($area))
     $areaname = $area_data['zone_id'] ? getAreaName($area_data['zone_id'],0).' ('.$area_data['name'].')<br>' : $area_data['name'].'<br>';
   else
     $areaname = '';
        if ($data['type']=='n') $name = getCreatureName($data['id'], 0);
   else if ($data['type']=='o') $name = getGameobjectName($data['id'], 0);
//   $text ="$name<br>GUID $data[guid]<br>map=$data[map]<br>x = $data[position_x]<br>y = $data[position_y]<br>z = $data[position_z]<br>Respawn: ".getTimeText($data['spawntimesecs']);
   $text ="$name ($data[guid])<br>{$areaname}Respawn: ".getTimeText($data['spawntimesecs']);
   $text = validateText($text);
   return '<img src="'.$img.'" class=point style="left: '.$x.'; top: '.$y.';" onmouseover="Tip(\''.$text.'\');">'."\n";
}
...
//   $text ="$name<br>GUID $data[guid]<br>$mapname - $areaname<br>x = $data[position_x]<br>y = $data[position_y]<br>z = $data[position_z]<br>map = $data[map]<br>Respawn: ".getTimeText($data['spawntimesecs']);
   $text ="$name ($data[guid])<br>$mapname - $areaname<br>Respawn: ".getTimeText($data['spawntimesecs']);
   $text = validateText($text);
   return '<img src="'.$img.'" class=point style="left: '.$x.'; top: '.$y.'; z-index: 1;" onmouseover="Tip(\''.$text.'\');">'."\n";
}
...
function renderGPSMap($header, $outSizeX, $pointsList = 0, $render = 'defaultMapRenderCallback')
{
  ...
  echo "<img src=$image width=$imageY height=$imageX>\n";
...
function renderArea($areaId, $outSizeX = 0, $pointsList = 0, $render = 'defaultAreaRenderCallback')
{
  ...
  echo "<tr><td width=$sizeY height=$sizeX align=left valign=top>";
  echo "<div id=mapperarea style=\"font-size: 10px; position: relative; border: 0px; left: 0; top: 0;\" onmousemove=outMouseCoords(this,event,'mappercoord') onmouseout=cleanMouseCoords('mappercoord')>";
  echo "<img src=$image width=$imageY height=$imageX>\n";
  if($pointsList)
  foreach ($pointsList as $point)
  {
...
function renderMap($mapId, $outSizeX = 0, $pointsList = 0, $render = 'defaultMapRenderCallback')
{
  ...
  echo "<tr><td width=$sizeY height=$sizeX align=left valign=top>";
  echo "<div id=mapperarea style=\"position: relative; border: 0px; left: 0; top: 0;\">";
  echo "<img src=$image width=$imageY height=$imageX>\n";
...
function createPointsList($npc_id, $obj_id)
{
 global $dDB;
 $list=array();
 if ($npc_id)
  ...
  'SELECT
   ...
   `DeathState`,
   `MovementType`
   FROM `creature` WHERE `id` = ?d', $npc_id);
 else if ($obj_id)
   return $dDB->select(
   'SELECT
   \'o\' AS `type`,
   ...
   `spawntimesecs`,
   `state`
   FROM `gameobject` WHERE `id` = ?d', $obj_id);
 return 0;
}

function getMapsListFromPoints($points)
{
   $areaList= array();
   $mapsList= array();
   ...
   if ($area_keys)
      foreach($areaList as $id=>$count)
         $list[]=array('type'=>'area', 'id'=>$id, 'text'=>getAreaNameFromId($id).' - ('.$count.')');
   if ($mapsList)
      foreach($mapsList as $id=>$count)
         $list[]=array('type'=>'map', 'id'=>$id, 'text'=>getMapName($id).' - ('.$count.')');
   return $list;
}
...
function getPointData($area_id, &$data, $x, $y)
{
   ...
   if (!$area_data)
       return 0;
   $zone     = @$area_data['zone_id'] ? $area_data['zone_id'] : $area;
   $areaname = validateText($area_data['zone_id'] ? getAreaName($zone)." (".$area_data['name'].")" : $area_data['name']);
