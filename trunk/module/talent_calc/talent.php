...
{
    echo '<a href="?talent='.$name.'" onmouseover="Tip(\''.getClass($c).'\')"><img class=item src="'.getClassImage($c).'"></a>&nbsp;';
    if ($talent==$name)
    {
        $header = getClass($c);
        $link.="=".$name;
...
echo "<br>";
//echo "<br><b>Pets:</b> ";
foreach($gHunterPetType as $c=>$family)
{
    echo '<a href="?talent&pet='.$c.'" onmouseover="Tip(\''.getCreatureFamily($family,0).'\')"><img class=item src="'.getFamilyImage($family).'"></a>&nbsp;';
    if ($pid==$c)
    {
        $header = getCreatureFamily($family);
        $link.="&pet=".$c;
    }
}
echo '</div><br>';
