...
else
{
  ...
  if ($ajaxmode==0)
  {
    ...
    renderFactionGroupList($faction, $rows, "Состоит из:", $page_seek, $totalRecords, $baseLink, 'teamLIST');
    // Входит в группу фракций
    if ($faction['team'])
    {
      ...
      renderFactionGroupList($parent, $rows, "Входит в состав:", $page_seek, $totalRecords, $baseLink, 'cteamLIST');
    }/**/
  }
  ...
  {
    $npc->inFaction($entry);
    $npc->createReport('Входят существа');
  }
  ...
  {
    $go->inFaction($entry);
    $go->createReport('Входят обьекты');
  }
  //********************************************************************************
  ...
  if ($item_req->Init($fields, $baseLink, 'itemreqLIST', $config['fade_limit'], 'rep_rank'))
  {
    ...
    $item_req->createReport('Вещи требующие репутации');
  }
  //********************************************************************************
  // Как можно получить репутацию:
  // Награда за квест
  ...
  if ($quest_list->Init($fields, $baseLink, 'questLIST', $config['fade_limit'], 'name'))
  {
    ...
    $quest_list->createReport("Награда за выполнение квеста");
  }
  //********************************************************************************
  // Как можно получить репутацию:
  // Награда за существо
  ...
  if ($spell_list->Init($fields, $baseLink, 'spellLIST', $config['fade_limit'], 'name'))
  {
    ...
    $spell_list->createReport("Получена от спелла");
  }
}
?>