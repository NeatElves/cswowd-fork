...
$mapId = intval(@$_REQUEST['instance']);
if ($mapId==0)
{
...
else
{
 ...
 if ($ajaxmode == 0)
 {
   ...
   if ($map = getRenderMapData($mapId))
   {
     ...
     // Обьекты
     //***********************************
     $go_list = $dDB->selectPage($totalRecords,
     "SELECT
      `guid`,
      `id`,
      `map`,
     ...
     foreach ($go_list as $point)
     {
           ...
           $areaX1 >= $posX AND $areaX2 <= $posX)
       {
         ...
         if (@$objCache[$point['id']] == 0)
             @$objCache[$point['id']] = getGameobject($point['id']);
         $o = $objCache[$point['id']];
         $id= "o".$point['id'];
     ...
     // Существа
     //***********************************
     $creatures_list = $dDB->selectPage($totalRecords,
     "SELECT
      `guid`,
      `id`,
      `map`,
     ...
     foreach ($creatures_list as $point)
     {
           ...
           $areaX1 >= $posX AND $areaX2 <= $posX)
       {
         $x = ($posX-$areaX1)/($areaX2-$areaX1)*100;
         $y = ($posY-$areaY1)/($areaY2-$areaY1)*100;
         if (@$crCache[$point['id']] == 0)
           @$crCache[$point['id']] = getCreature($point['id']);
         $c = $crCache[$point['id']];
         $id= "c".$point['id'];
         $mapdata['points'][$i]['id']=$id;
         $mapdata['points'][$i]['x']=$y;
