...
function dumpRep($depth, $tree, &$rep)
{
    ...
    foreach($tree as $id=>$t)
    {
      if ($t['details'])
      {
        $tip = '<table class=skilltip><tr class=top><td>'.validateText($t['name']).'</td></tr><tr><td>'.validateText($t['details']).'</td></tr></table>';
        echo '<tr onmouseover="Tip(\''.$tip.'\', WIDTH, 400, STICKY, false, BORDER, false);">';
      }
      else
        echo '<tr>';
      if (isset($t['childs']))
