...
define('QUEST_MANGOS_FLAGS_EXPLORATION_OR_EVENT',0x020000);     // Set by 2 in SpecialFlags from DB (if required area explore, spell SPELL_EFFECT_QUEST_COMPLETE casting, table `*_script` command SCRIPT_COMMAND_QUEST_EXPLORED use, set from script DLL)
define('QUEST_MANGOS_FLAGS_DELIVER',             0x040000);     // Internal flag computed only
define('QUEST_MANGOS_FLAGS_SPEAKTO',             0x080000);     // Internal flag computed only
define('QUEST_MANGOS_FLAGS_KILL_OR_CAST',        0x100000);     // Internal flag computed only
define('QUEST_MANGOS_FLAGS_TIMED',               0x200000);     // Internal flag computed only
?>