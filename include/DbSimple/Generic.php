...
class DbSimple_Generic_Database extends DbSimple_Generic_LastError
{
    ...
    function _query($query, &$total)
    {
        ...
        if (!empty($this->attributes['CACHE']) && $is_cacher_callable) {
            ...
            if ($ttl && $uniq_key == $invalCache) {
                ...
                $this->_logQueryStat($queryTime, $fetchTime, $firstFetchTime, $rows, true);
        ...
        if (null === $rows || true === $cache_it) {
            ...
            $this->_logQueryStat($queryTime, $fetchTime, $firstFetchTime, $rows, false);
    ...
    /**
     * void _logQueryStat($queryTime, $fetchTime, $firstFetchTime, $rows, $cached)
     * Log information about performed query statistics.
     */
    function _logQueryStat($queryTime, $fetchTime, $firstFetchTime, $rows, $cached)
    {
        // Always increment counters.
        $this->_statistics['time'] += $queryTime;
        $this->_statistics['count']++;
		if ($cached) $this->_statistics['cache']++;
        ...
        if ($firstFetchTime + $tailFetchTime) {
            $log = sprintf(($cached?'> <b>cached</b>':'')."  -- %d ms = %d+%d".($tailFetchTime? "+%d" : ""), $dt, $dt-$firstFetchTime-$tailFetchTime, $firstFetchTime, $tailFetchTime);
        } else {
            $log = sprintf(($cached?'> <b>cached</b>':'')."  -- %d ms", $dt);
        }
        $log .= "; returned ";
